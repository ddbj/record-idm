...
| 次元               | 意味                       | 値                                                                                |
| ------------------ | -------------------------- | --------------------------------------------------------------------------------- |
| `record_status`    | データの公開状態           | `unpublished`, `live`, `suppressed`, `withdrawn`, `canceled`, `unregistered`      |
| `submission_stage` | 投稿処理の段階（nullable） | `draft`, `submitted`, `in_curation`, `revision_requested`, `accepted`, `rejected` |

`record_status` の前半 4 値（unpublished / live / suppressed / withdrawn）は INSDC 標準と互換。`canceled` / `unregistered` は DDBJ 拡張。
